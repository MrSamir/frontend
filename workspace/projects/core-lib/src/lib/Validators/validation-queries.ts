...
export function isElementBlank(form: FormGroup, elementName: string) {
  let element = form.controls[elementName];
  return !!element && (element.value === '' || element.value === undefined || element.value === null);
}


export function Rangvalidator(controlName: string, From: number,To:number, elementName: string) : boolean | null {
  // @ts-ignore
